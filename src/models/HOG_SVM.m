...
[eigenVectors, eigenValues, meanX, Xpca] = PrincipalComponentAnalysis(trHogs);

svmModel = fitcsvm(Xpca,trLabels);
...
video = VideoWriter('models/HOG_SVM_PCA.avi');
video.FrameRate = 1;
open(video);
        ...
        for rowIndex=1:20:imHeight
            for columnIndex=1:20:imWidth
                ...
                if (heightFit && widthFit)
                    ...
                    winHog = (hog_feature_vector(hWindow)-meanX)*eigenVectors;
                    [label,score] = predict(svmModel, winHog);
                    if (label == 1)
        ...
        for columnIndex = 1:20:imWidth
            for rowIndex = 1:20:imHeight
                ...
                if (widthFit && heightFit)
                    ...
                    winHog = (hog_feature_vector(vWindow)-meanX)*eigenVectors;
                    [label,score] = predict(svmModel, winHog);
                    if (label == 1)
                        positions = [positions; [score(1), columnIndex, rowIndex, winW, winH]];
                    end
...
close(video);
toc
save('HOG_SVM_DETECTOR','modelDetections');
compareDetections(modelDetections,'HOG_SVM');
