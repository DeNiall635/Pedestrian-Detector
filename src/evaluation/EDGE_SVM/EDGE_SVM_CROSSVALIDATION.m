...
trainingDataset = horzcat(trLabels, trEdges);
...
for h = 1:size(foldValues,2)
    ...
    for i = 1:testSize:datasetSize
        ...
        for c = 1:length(cValues)
            ...
            graphName = sprintf('EDGE_SVM - c = %i - %i Fold CV - ROC Curve', cValue, nFolds);
            ...
            for j = 1:size(testImages,1)
                ...
                tsImEdge = (testImages(j,:) - meanX) * eigenVectors;
                [label, score] = predict(svmModel,tsImEdge);
                
                % store prediction
                modelPredictions(1,j) = label;
            end
            
            % evaluate accuracy of model etc.
...
% set table data
tblHeadings = {"C", "No. Folds", "This Fold", "Accuracy", "Error Rate", "Sensitivty", "Precision", "Specificity", "False Alarm Rate", "F-Measure", "AUC", "Time"};
tblData = horzcat(cVals, nFold, cFold, accuracies, errorRates, sensitivities, precisions, specificities, falseAlarmRates, fMeasures, aucs, cTimes);
...
writetable(tbl,'reportData/evaluationResults/edge_svm_cross-validated','Delimiter',',', 'WriteRowNames', true);
