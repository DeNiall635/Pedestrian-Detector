...
for k = 1:length(kValues)
    ...
    sprintf("%i nearest-neighbour classification", knn)
    ...
    for f = 1:length(foldValues)
        ...
        rocName = sprintf("HOG_KNN - %inn - %i fold cv - ROC Curve", knn, nFolds);
        ...
        for i = 1:testSize:datasetSize
            ...
            % createModel
            knnModel = fitcknn(Xpca, trainingLabels, 'NumNeighbors', knn);
...
tblHeadings = {"NN", "N Folds", "Accuracy", "Error Rate", "Sensitivty", "Precision", "Specificity", "False Alarm Rate", "F-Measures", "AUC", "Time"};
tblData = horzcat(nnValues, nFold, accuracies, errorRates, sensitivities, precisions, specificities, falseAlarmRates, fMeasures, aucs, cTimes);
uitable("Data", tblData, "ColumnName", tblHeadings);
disp(tblData);

% save evaluation metrics into a csv .txt
tbl = table(nnValues, nFold, accuracies, errorRates, sensitivities, precisions, specificities, falseAlarmRates, fMeasures, aucs, cTimes);
tbl.Properties.VariableNames = {'NN', 'N Folds', 'Accuracy', 'Error Rate', 'Sensitivty', 'Precision', 'Specificity', 'False Alarm Rate', 'F-Measures', 'AUC', 'Time'};
writetable(tbl,'reportData/evaluationResults/hog_knn_cross-validated','Delimiter',',', 'WriteRowNames', true);

% save raw metrics into a csv .txt
tbl = table(nnValues, nFold, Totals, TPs, TNs, FPs, FNs);
tbl.Properties.VariableNames = {'NN', 'N Folds', 'No. Observations','TP','TN','FP','FN'};
writetable(tbl, 'reportData/evaluationResults/rawResults/hog_knn_cross-validated_raw', 'Delimiter', ',', 'WriteRowNames', true);
